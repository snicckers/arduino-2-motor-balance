...
void calculate_attitude_xio(int sensor_data[]){
  ...
  float a_y = sensor_data[1];
  float a_z = sensor_data[2];
  normalize = fstInvSqrt(a_x*a_x + a_y*a_y + a_z*a_z);
  a_x *= normalize; a_y *= normalize; a_z *= normalize;
  ...
  normalize = fstInvSqrt(q_0*q_0 + q_1*q_1 + q_2*q_2 + q_3*q_3);
  q_0 *= normalize; q_1 *= normalize; q_2 *= normalize; q_3 *= normalize;
...
void calculate_attitude_sqrt2(int sensor_data[]){
  /*--- Madgwick Filter ------------------------------------------------------*/
  float normalize;
  ...
  float a_y = sensor_data[1];
  float a_z = sensor_data[2];
  normalize = sqrt2(a_x*a_x + a_y*a_y + a_z*a_z);
  a_x /= normalize; a_y /= normalize; a_z /= normalize;
  ...
  normalize = sqrt2(q_0*q_0 + q_1*q_1 + q_2*q_2 + q_3*q_3);
  q_0 /= normalize; q_1 /= normalize; q_2 /= normalize; q_3 /= normalize;
